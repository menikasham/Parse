...
logging.basicConfig(filename='adm_parse.log', level=logging.DEBUG,
    ...
    for _ in range(len(rows)):
        ...
        if not rows[i].find('td').get_text():
            ...
            print_houses.append({
                ...
                'link_adm': rows[i].find('a').get('href').split('//')[
                                0] + '//adm_' +
...
def insert_into_db(dictionary):
    ...
    for i in range(0, len(dictionary)):
        try:
            sql = (f"INSERT INTO Customers (web_id_link, adm_link) VALUES("
...
def save_result(items, path):
    with open(path, 'w', newline='') as file:
        ...
        for item in items:
            writer.writerow([item['name'], item['webid'], item['webid_link'],
                             item['link_adm']])
